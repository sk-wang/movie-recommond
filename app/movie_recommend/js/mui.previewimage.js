(function($, window) {
	...
	proto.init = function() {
		...
		if (!el) {
			...
			window.parent.document.body.appendChild(div.firstElementChild);
			el = window.parent.document.getElementById(this.options.id);
		}
	...
	proto._initImgData = function(itemData, imgEl) {
		if (!itemData.sWidth) {
			...
			itemData.sScale = Math.max(itemData.sWidth / window.parent.innerWidth, itemData.sHeight / window.parent.innerHeight);
		}
	...
	proto._getPosition = function(itemData) {
		var sLeft = itemData.sLeft - window.parent.pageXOffset;
		var sTop = itemData.sTop - window.parent.pageYOffset;
		var left = (window.parent.innerWidth - itemData.sWidth) / 2;
		var top = (window.parent.innerHeight - itemData.sHeight) / 2;
		return {
			left: sLeft,
			top: sTop,
			x: sLeft - left,
	...
	proto.close = function(index, group) {
		...
		if (imgEl) {
			...
			var sLeft = posi.left;
			var sTop = posi.top;
			if (sTop > window.parent.parinnerHeight || sLeft > window.parent.innerWidth || sTop < 0 || sLeft < 0) { //out viewport
